...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.RangeSlider(id='payload-slider',min=0, max=10000, step=1000,marks={0: '0', 100: '10000'},value=[min_payload, max_payload]),
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='class', 
        title='Total success for sites'+entered_site)
        return fig
# TASK 4:
...
def chartb(entered_site,range):
    min_payload=range[0]
    max_payload=range[1]
    ranged_df=spacex_df[spacex_df['Payload Mass (kg)'].between(min_payload,max_payload)]
    filtered_df = ranged_df
    if entered_site == 'ALL':
        ...
        fig2 = px.scatter(filtered_df,
        x='Payload Mass (kg)', y='class',color="Booster Version Category", title='Correlation between Payload and Success for'+entered_site)
        return fig2
